...
// localStorage読み込み
const getToDoDataFromlocalStorage = (): void /*abstData[]*/ => {
...
const afterToDoDelete = (): void => {
    ...
    if (GET_TO_DO_DATA_LIST == 1)    // localstorageに保存
        accessLocalStorage(-100, "TEMP", "TEMP");
...
//  1 set key and value
//  2 get value
const accessLocalStorage = (action: number, key: string, value: string): string => {

    /* keyとvalueのセットでストレージに保存される */
    /* clearしない限り永続的にストレージにキープされ続ける */
